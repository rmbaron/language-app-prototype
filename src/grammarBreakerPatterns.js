...
import { L2_FIELDS_SET } from './L2Fields'
...
function validatePatternRecord(p, seenIds) {
  ...
  // consumesL2Fields validation: every declared field must be a recognized
  // L2 field. Catches typos and fields that aren't generated by the L2
  // enricher prompt. The full L2 list lives in src/L2Fields.js — keep in
  // sync when the enricher prompt changes.
  if (p.consumesL2Fields != null) {
    if (!Array.isArray(p.consumesL2Fields)) {
      errors.push('consumesL2Fields must be an array of field-name strings')
    } else {
      const bad = p.consumesL2Fields.filter(f => !L2_FIELDS_SET.has(f))
      if (bad.length > 0) {
        errors.push(`consumesL2Fields has unrecognized field(s): ${bad.join(', ')} (not in L2_FIELDS — add to src/L2Fields.js if newly generated by the enricher)`)
      }
    }
  }
  return errors
